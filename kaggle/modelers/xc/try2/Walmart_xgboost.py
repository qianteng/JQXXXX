...
from walmart_preprocess import walmart_prepare
...
"""Preparing Walmart training set for XGBoost"""
random.seed(a=0)
...
"""Load Walmart test sets and make predictions"""
dtest1 = xgb.DMatrix('test1.buffer')
dtest2 = xgb.DMatrix('test2.buffer')
...
"""Writing to csv"""
test1 = pd.read_csv('test.csv').ix[rows]
test2 = pd.read_csv('test.csv').drop(rows)
...
out11 = pd.DataFrame(pred11, index=visit_num1).reset_index()
out11.columns = col_names
out21 = pd.DataFrame(pred21, index=visit_num1).reset_index()
out21.columns = col_names
out12 = pd.DataFrame(pred12, index=visit_num2).reset_index()
out12.columns = col_names
out22 = pd.DataFrame(pred22, index=visit_num2).reset_index()
out22.columns = col_names

out = pd.concat([out11, out21, out12, out22]).groupby('VisitNumber').mean()

out.to_csv('Walmart_xgboost.csv')
